...
#- Uncomment for Human genome
canonical_transcript="knownCanonical_filterTaggedGenes.txt"  #- these have tagged genes removed
exons_file="wgEncodeGencodeBasicV40.tsv"
genome="human/hs"

#- Uncomment for Mouse genome
#canonical_transcript="knownCanonical_filterTaggedGenes.Mm.txt"  #- these have tagged genes removed
...
#- open canonical transcripts (but exclude non canonical chromosomes)
...
#- get exons in canonical transcripts only
...
#- this contains a list of exons or introns per gene (using only their canonical transcript), excluding tagged genes 
write.table(exon, paste0(genome, "_bedfiles/exon_filtered.bed"), sep="\t", row.names=F, col.names=F, quote=F)
write.table(intron, paste0(genome, "_bedfiles/intron_filtered.bed"), sep="\t", row.names=F, col.names=F, quote=F)
