...
for (dir in c("human/bigwigs","mouse/bigwigs")){
	...
	#– This is just to get the sample names
	...
	for (sample in samples){
		if ((paste0(sample, "-", "PlusTag") %in% samples2) && (paste0(sample, "-", "MinusTag") %in% samples2)){
			...
			# plot exon and intron segments together
			type = "exonintron"
			...
			#- make a scatterplot, use pseudolog transformation
			...
			p <- ggplot(file, aes(x = mean_m, y = mean_p)) +
			  ...
			  theme(
			    axis.title = element_text(size = 32, family = "Arial"),
		...
		if ((str_split_fixed(sample, "_", n=2)[1] != "GFP") && 
			...
			#- scatterplot, use psuedilog transformation
